    ...
    <>
      <div className="mb-6">
        ...
        <h1 className="text-[26px] font-semibold mt-1.5">
          {saludo()}, <span className="text-[#2f6bff]">{u.nombre.split(" ")[0]}</span>
        </h1>
      ...
      {enRojo > 0 && (
        <Link href="/vehiculos?alerta=rojo"
          className="mt-3 flex items-center gap-3 rounded-xl border border-[#991b1b] bg-[#2e0a0a]
            px-4 py-3 hover:bg-[#3a0d0d] transition-colors">
          ...
          <span className="ml-auto text-[12px] text-[#f87171] shrink-0">Ver →</span>
        </Link>
      )}
      ...
      <div className="grid lg:grid-cols-2 gap-4 mt-4">
        ...
        <Panel>
          ...
          {porSucursal.length === 0 ? (
            <p className="text-[13px] text-[#64748b] py-6 text-center">Todavía no hay unidades cargadas.</p>
          ) : (
              ...
              {porSucursal.map((r: any) => (
                <div key={r.sucursal}>
                  <div className="flex items-baseline justify-between mb-1.5">
                    ...
                    <span className="text-[13px] tabular text-[#9aa7b8]">
                      {r.n}{costos && <span className="text-[#64748b]"> · {plataCorta(r.capital)}</span>}
                    </span>
                  </div>
                  <div className="h-1.5 rounded-full bg-[#1f2937] overflow-hidden">
                    <div className="h-full rounded-full bg-[#2f6bff]"
        ...
        <Panel>
          ...
          {porEstado.length === 0 ? (
            <p className="text-[13px] text-[#64748b] py-6 text-center">Sin datos.</p>
          ) : (
              ...
              {porEstado.map((r: any) => {
                ...
                return (
                  <Link key={r.estado} href={`/vehiculos?estado=${r.estado}`}
                    className="flex items-center gap-2.5 rounded-lg border border-[#1f2937]
                      bg-[#0f1520] px-3.5 py-2.5 hover:border-[#2f6bff] transition-colors">
      ...
      <div className="grid lg:grid-cols-2 gap-4 mt-4">
        ...
        <Panel>
          ...
          {ultimos6.length === 0 ? (
            <p className="text-[13px] text-[#64748b] py-10 text-center">Todavía no hay ventas cerradas.</p>
          ) : (
            <div className="flex items-end gap-3 h-[150px] pt-2">
              {ultimos6.map((r: any) => (
                <div key={r.mes} className="flex-1 flex flex-col items-center gap-2">
                  <span className="text-[12px] tabular text-[#9aa7b8]">{r.n}</span>
                  <div className="w-full rounded-t-md bg-[#2f6bff] min-h-[3px]"
                       style={{ height: `${(r.n / maxMes) * 100}%` }} />
                  <span className="text-[11px] text-[#64748b] capitalize">{r.mes}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <Panel>
          ...
          {porVendedor.length === 0 ? (
            <p className="text-[13px] text-[#64748b] py-10 text-center">Sin operaciones en el período.</p>
          ) : (
              ...
              {porVendedor.map((r: any) => (
                <div key={r.vendedor}>
                  <div className="flex items-baseline justify-between mb-1.5">
                    ...
                    <span className="text-[13px] tabular text-[#9aa7b8]">{r.n}</span>
                  </div>
                  <div className="h-1.5 rounded-full bg-[#1f2937] overflow-hidden">
        ...
        <Panel padding={false}>
          <div className="p-5 pb-0">
            <PanelTitulo titulo="Unidades demoradas" detalle="Más de 60 días en el predio"
              accion={<Link href="/vehiculos" className="text-[12.5px] text-[#2f6bff] hover:underline">Ver stock</Link>} />
          </div>
                  ...
                  <tr key={v.id} className="hover:bg-[#151d29]">
                    <TD>
                      <Link href={`/vehiculos/${v.id}`} className="hover:text-[#2f6bff]">
                        ...
                        <div className="text-[11.5px] text-[#64748b]">{dominio(v.dominio)} · {v.anio}</div>
                      </Link>
                    </TD>
                    <TD alinear="right"><Chip tono={a.tono}>{v.dias_stock} días</Chip></TD>
                    {costos && <TD alinear="right" className="text-[#9aa7b8]">{plataCorta(v.costo_total)}</TD>}
                  </tr>
                );
              })}
            </tbody>
        ...
        <Panel padding={false}>
          <div className="p-5 pb-0">
            <PanelTitulo titulo="Próximas acciones" detalle="Leads que hay que seguir"
              accion={<Link href="/leads" className="text-[12.5px] text-[#2f6bff] hover:underline">Ver leads</Link>} />
          </div>
          <Tabla className="border-0 rounded-none bg-transparent">
            <tbody>
              ...
              {acciones.map((l: any) => {
                ...
                return (
                  <tr key={l.id} className="hover:bg-[#151d29]">
                    <TD>
                      <Link href={`/leads/${l.id}`} className="hover:text-[#2f6bff]">
                        <div className="font-medium">{l.nombre}</div>
                        <div className="text-[11.5px] text-[#64748b] truncate max-w-[220px]">
