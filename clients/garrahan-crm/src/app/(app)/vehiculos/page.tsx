...
export default async function Vehiculos({ searchParams }: { searchParams: Promise<any> }) {
  ...
  const celda = (v: any, clave: string) => {
    ...
    switch (clave) {
      case "dominio":
        return (
          <Link href={`/vehiculos/${v.id}`} className="font-semibold text-[var(--c-enlace)] hover:underline">
      ...
      case "vehiculo":
        return (
          <Link href={`/vehiculos/${v.id}`} className="hover:text-[var(--c-primario)]">
            ...
            {v.version && <div className="text-[11.5px] text-[var(--c-tinta-tenue)]">{v.version}</div>}
          </Link>
        );
      case "anio": return <span className="text-[var(--c-tinta-media)]">{v.anio || "—"}</span>;
      case "km": return <span className="text-[var(--c-tinta-media)]">{fkm(v.km)}</span>;
      case "costos": return <span className="text-[var(--c-tinta-media)]">{Number(v.costos) > 0 ? plata(v.costos) : "—"}</span>;
      ...
      case "margen":
        return Number(v.precio_venta) > 0 ? (
          <span className={Number(v.margen) >= 0 ? "text-[var(--c-verde)]" : "text-[var(--c-rojo-alto)]"}>
            {porcentaje(v.margen_pct)}
          </span>
        ) : "—";
      case "dias":
        return a ? <Chip tono={a.tono}>{v.dias_stock}</Chip> : <span className="text-[var(--c-tinta-tenue)]">—</span>;
      case "ubicacion":
        return (
          <span className="text-[var(--c-tinta-media)]">
            {v.sucursal || "—"}
            {v.propiedad !== "propia" && (
              <span className="block text-[11px] mt-0.5"
                style={{ color: v.propiedad === "consignacion" ? "var(--c-celeste)" : "var(--c-violeta-alto)" }}>
    ...
    !["vendido", "baja"].includes(v.estado) ? (
      <span className="inline-flex items-center gap-1.5">
        <Link href={`/taller/nueva?vehiculo=${v.id}`} title="Mandar al taller"
          className="inline-flex items-center gap-1.5 rounded-md border border-[var(--c-naranja-borde)]
            bg-[var(--c-naranja-fondo)] px-2 py-1 text-[11.5px] text-[var(--c-naranja-alto)]
            hover:bg-[var(--c-naranja-fondo-alto)] transition-colors whitespace-nowrap">
    ...
    <>
      ...
      <div className="no-imprimir flex flex-wrap items-center gap-1.5 mt-6 mb-3">
        {TABS.map((t) => (
          <Link key={t.valor} href={link({ tab: t.valor })}
            className={`rounded-lg px-3 py-1.5 text-[12.5px] font-medium transition-colors
              ${tab === t.valor ? "bg-[var(--c-activo)] text-[var(--c-tinta)]" : "text-[var(--c-tinta-media)] hover:bg-[var(--c-hover)]"}`}>
            {t.label}
            <span className="ml-1.5 text-[11px] text-[var(--c-tinta-tenue)] tabular">{(c as any)[t.valor] ?? 0}</span>
          </Link>
        ))}
        ...
        <div className="ml-auto flex items-center gap-1.5">
          ...
          {vista === "tabla" && (
            <details className="relative">
              <summary className="cursor-pointer list-none inline-flex items-center gap-1.5
                rounded-lg border border-[var(--c-borde)] bg-[var(--c-panel)] px-3 py-1.5
                text-[12.5px] text-[var(--c-tinta-clara)] hover:border-[var(--c-borde-alto)] transition-colors">
                <Columns3 size={14} /> Columnas
                <span className="text-[var(--c-tinta-tenue)] tabular">{visibles.length}</span>
              </summary>
              <form action="/vehiculos" className="absolute right-0 z-20 mt-2 w-[230px] p-3
                bg-[var(--c-panel)] border border-[var(--c-borde)] rounded-xl shadow-2xl">
                  ...
                  {permitidas.map((col) => (
                    <label key={col.clave}
                      className={`flex items-center gap-2.5 text-[12.5px] cursor-pointer
                        ${col.fija ? "text-[var(--c-tinta-tenue)]" : "text-[var(--c-tinta-clara)]"}`}>
                      <input type="checkbox" name="cols" value={col.clave}
                        defaultChecked={visibles.some((x) => x.clave === col.clave)}
                        disabled={col.fija}
                        className="h-3.5 w-3.5 accent-[var(--c-primario)]" />
                      {col.label}
                      {col.fija && <span className="text-[10.5px]">(siempre)</span>}
                    </label>
                  ))}
                </div>
                <button type="submit" className="mt-3 w-full rounded-lg bg-[var(--c-primario)]
                  hover:bg-[var(--c-primario-alto)] py-1.5 text-[12.5px] font-semibold text-white transition-colors">
                  Aplicar
                </button>
              </form>
            </details>
          ...
          {/* ------------------------------------------------- tabla o tarjetas */}
          <div className="flex rounded-lg border border-[var(--c-borde)] bg-[var(--c-panel)] overflow-hidden">
            {[
              ...
              <Link key={v} href={link({ vista: v === "tabla" ? "" : v })} title={titulo}
                className={`px-2.5 py-2 transition-colors
                  ${vista === v ? "bg-[var(--c-activo)] text-[var(--c-tinta)]" : "text-[var(--c-tinta-tenue)] hover:text-[var(--c-tinta-clara)]"}`}>
                <Ico size={15} />
              </Link>
            ))}
          </div>
      ...
      <form className="no-imprimir flex flex-wrap gap-2 mb-3" action="/vehiculos">
        ...
        <div className="relative flex-1 min-w-[220px]">
          ...
          <kbd className="absolute right-3 top-1/2 -translate-y-1/2 text-[10.5px] px-1.5 py-0.5
            rounded border border-[var(--c-borde)] bg-[var(--c-superficie)] text-[var(--c-tinta-apagada)] pointer-events-none">/</kbd>
        </div>
      ...
      {vista === "tabla" ? (
        ...
        <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-3">
          ...
          {filas.map((v: any) => {
            ...
            const a = v.alerta ? ALERTAS[v.alerta] : null;
            return (
              <Panel key={v.id} className="hover:border-[var(--c-borde-alto)] transition-colors">
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="min-w-0">
                    <Link href={`/vehiculos/${v.id}`}
                      className="font-semibold text-[14px] hover:text-[var(--c-primario)] block truncate">
                      {v.marca} {v.modelo}
                    </Link>
                    <div className="text-[11.5px] text-[var(--c-tinta-tenue)] truncate">
                ...
                <div className="flex items-baseline justify-between gap-3 mb-3">
                  <Link href={`/vehiculos/${v.id}`}
                    className="text-[12.5px] font-semibold text-[var(--c-enlace)] hover:underline">
                ...
                <div className="flex items-center justify-between gap-3 pt-3 border-t border-[var(--c-borde)]">
                  <span className="text-[11.5px] text-[var(--c-tinta-tenue)] truncate">
                    {v.sucursal || "Sin sucursal"}
                    {v.propiedad === "consignacion" && (
                      <span className="text-[var(--c-celeste)]"> · En consignación</span>
                    )}
                    {v.propiedad === "inversor" && (
                      <span className="text-[var(--c-violeta-alto)]"> · {v.inversor}</span>
                    )}
                ...
                {costos && Number(v.precio_venta) > 0 && (
                  <div className="flex items-center justify-between gap-3 mt-2 text-[12px]">
                    <span className="text-[var(--c-tinta-tenue)]">Margen</span>
                    <span className={Number(v.margen) >= 0 ? "text-[var(--c-verde)]" : "text-[var(--c-rojo-alto)]"}>
      ...
      {filas.length >= 300 && (
        <p className="text-[12px] text-[var(--c-tinta-tenue)] mt-3">Mostrando las primeras 300 unidades. Afiná la búsqueda para ver el resto.</p>
      )}
    </>
  );
}
