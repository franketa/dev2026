...
export default async function Login({ searchParams }: { searchParams: Promise<{ error?: string; m?: string }> }) {
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2">
      {/* ---------------------------------------------------------- marca */}
      <div className="hidden lg:flex flex-col justify-between p-12 bg-gradient-to-br from-[var(--c-degradado-a)] via-[var(--c-fondo)] to-[var(--c-fondo)] border-r border-[var(--c-borde)]">
        ...
        <div>
          <h1 className="text-[42px] leading-[1.1] font-semibold tracking-tight">
            Toda la agencia,<br />
            <span className="text-[var(--c-primario)]">en un solo lugar.</span>
          </h1>
          <p className="mt-5 text-[14px] text-[var(--c-tinta-media)] max-w-md leading-relaxed">
          ...
          <div className="mt-8 flex flex-wrap gap-2">
            {["Stock", "Ventas", "Leads", "Taller", "Caja", "Inversores"].map((t) => (
              <span key={t} className="rounded-full border border-[var(--c-borde)] bg-[var(--c-panel)] px-3 py-1.5 text-[12px] text-[var(--c-tinta-media)]">{t}</span>
            ))}
          </div>
        </div>
        <p className="text-[12px] text-[var(--c-tinta-apagada)]">Chivilcoy, Provincia de Buenos Aires</p>
      </div>
      ...
      <div className="flex items-center justify-center p-6">
        <div className="w-full max-w-[380px]">
          ...
          <p className="text-[13px] text-[var(--c-tinta-tenue)] mt-1">Entrá con tu cuenta para seguir operando.</p>
          ...
          <form action={entrar} className="mt-7 space-y-4">
            ...
            {error && (
              <div className="rounded-lg border border-[var(--c-rojo-borde)] bg-[var(--c-rojo-fondo)] px-3 py-2.5 text-[12.5px] text-[var(--c-rojo-alto)]">
                Email o contraseña incorrectos.
              </div>
            )}

            {m === "clave" && (
              <div className="rounded-lg border border-[var(--c-verde-borde)] bg-[var(--c-verde-fondo)] px-3 py-2.5 text-[12.5px] text-[var(--c-verde-alto)]">
                Contraseña cambiada. Entrá con la nueva.
              </div>
            )}

            <button type="submit"
              className="w-full rounded-lg bg-[var(--c-primario)] hover:bg-[var(--c-primario-alto)] py-2.5 text-[14px] font-semibold text-white transition-colors">
              Ingresar
            </button>
          </form>

          <p className="mt-8 text-[11.5px] text-[var(--c-tinta-apagada)] text-center">
