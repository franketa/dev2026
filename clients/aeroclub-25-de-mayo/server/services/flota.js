...
const { ErrorNegocio, hoy, periodoActual, esFecha, fmtPesos, importeVuelo, limpiarTexto } = require('../util');
...
function listarAviones({ incluirInactivos = false } = {}) {
  ...
  return aviones.map(a => {
    ...
    return {
      ...
      mes_decimas: mes.d,
      mes_vuelos: mes.n,
      total_decimas: total.d,
      ultimo_vuelo: total.ultimo
    };
  });
}
...
function guardarAvion(input, actor, id = null) {
  ...
  try {
    if (id == null) {
      const r = db.prepare('INSERT INTO aviones (matricula, modelo, orden) VALUES (?, ?, (SELECT COALESCE(MAX(orden),0)+1 FROM aviones))')
        .run(matricula, modelo);
      auditar(actor.id, 'avion.alta', `${matricula} ${modelo}`);
      return Number(r.lastInsertRowid);
    }
    const antes = getAvion(id);
    db.prepare('UPDATE aviones SET matricula = ?, modelo = ?, activo = ? WHERE id = ?').run(matricula, modelo, activo, id);
    auditar(actor.id, 'avion.edicion', { antes, despues: { matricula, modelo, activo } });
    return id;
  } catch (e) {
...
module.exports = { tarifaVigente, tarifasVigentes, getAvion, listarAviones, guardarAvion, nuevaTarifa, historialTarifas };
